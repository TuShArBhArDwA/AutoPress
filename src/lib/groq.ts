  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (isRateLimit || isServerError) {
        ...
        const baseBackoff = Math.min(2000 * Math.pow(2, attempt), 16000);
...
`;
  ...
  const metaCompletion = await getGroqClient().chat.completions.create({
...
- Strip all conversational filler ("Here is the article:", etc.)`;
  ...
  const bodyCompletion = await getGroqClient().chat.completions.create({
