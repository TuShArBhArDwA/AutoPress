...
function SkeletonCard({ tall = false }: { tall?: boolean }) {
  return (
    <div className="news-card p-5 animate-pulse" style={{ height: tall ? '340px' : '220px' }}>
      <div className="flex gap-2 mb-4">
        <div className="h-5 w-14 rounded-full" style={{ background: 'var(--border)' }} />
        <div className="h-5 w-10 rounded-full" style={{ background: 'var(--border)' }} />
      </div>
      <div className="space-y-2 mb-4">
        <div className="h-5 rounded" style={{ background: 'var(--border)', width: '90%' }} />
        <div className="h-5 rounded" style={{ background: 'var(--border)', width: '75%' }} />
        {tall && <div className="h-5 rounded" style={{ background: 'var(--border)', width: '60%' }} />}
      </div>
      <div className="space-y-1.5">
        <div className="h-3.5 rounded" style={{ background: 'var(--border)', width: '100%' }} />
        <div className="h-3.5 rounded" style={{ background: 'var(--border)', width: '85%' }} />
        <div className="h-3.5 rounded" style={{ background: 'var(--border)', width: '70%' }} />
      </div>
    </div>
  );
}

function SkeletonWire() {
  return (
    <div className="flex items-start gap-4 py-4 animate-pulse" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
      <div className="w-8 h-6 rounded" style={{ background: 'var(--border)', flexShrink: 0 }} />
      <div className="flex-1 space-y-2">
        <div className="flex gap-2">
          <div className="h-4 w-14 rounded-full" style={{ background: 'var(--border)' }} />
          <div className="h-4 w-10 rounded-full" style={{ background: 'var(--border)' }} />
        </div>
        <div className="h-4 rounded" style={{ background: 'var(--border)', width: '85%' }} />
        <div className="h-3 rounded" style={{ background: 'var(--border)', width: '65%' }} />
      </div>
    </div>
  );
}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      >
        ...
        <span className="text-sm" style={{ color: 'var(--foreground-muted)', fontFamily: 'monospace' }}>
          {count > 0
            ? `Editorial pipeline running — ${count} report${count !== 1 ? 's' : ''} ready, generating more...`
            : 'AutoPress Editorial Engine initializing — synthesizing today\'s top reports...'}
        </span>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass-panel p-5">
            {[0, 1, 2].map(i => <SkeletonWire key={i} />)}
          </div>
        </div>
        <div className="space-y-6">
          ...
          <div className="glass-panel p-5 animate-pulse space-y-3">
            {[0, 1, 2, 3].map(i => (
              <div key={i} className="h-4 rounded" style={{ background: 'var(--border)', width: `${85 - i * 10}%` }} />
            ))}
          </div>
        </div>
      </div>
...
function HeroCard({ article }: { article: GeneratedArticle }) {
  ...
  const lede = paragraphs.length > 0 ? paragraphs[0].slice(0, 240) + '...' : article.subheadline;
    ...
    <article className="news-card-featured group relative animate-fade-in-up">
      ...
      <div className="p-6 sm:p-8">
        ...
        <Link href={`/article/${article.slug}`} className="block group/title">
          <h2 className="font-serif text-3xl sm:text-4xl font-bold leading-tight mb-3 transition-colors" style={{ color: 'var(--foreground)' }}>
            {article.headline}
          </h2>
        </Link>
        ...
        <div className="flex items-center justify-between pt-4" style={{ borderTop: '1px solid var(--border)' }}>
          <div className="flex items-center gap-4 text-xs" style={{ color: 'var(--foreground-dim)', fontFamily: 'monospace' }}>
            ...
            {(article.wordCount && article.wordCount > 50) ? <span>· {article.wordCount.toLocaleString()} words</span> : null}
          </div>
          <Link href={`/article/${article.slug}`} className="text-sm font-medium transition-colors flex items-center gap-1" style={{ color: 'var(--accent)' }}>
            Read full report
...
function SecondaryCard({ article, delay = 0 }: { article: GeneratedArticle; delay?: number }) {
  const priority = getPriorityBadge(article.editorialPriority);
  return (
    <article className={`news-card group p-5 animate-fade-in-up delay-${delay}`} style={{ height: '100%' }}>
      {article.imageUrl && (
        <div className="aspect-[16/7] overflow-hidden rounded-lg mb-4">
          <SafeImage src={article.imageUrl} alt="" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
        </div>
      )}
      ...
      <div className="flex items-center justify-between mt-auto">
        ...
        <Link href={`/article/${article.slug}`} className="text-xs font-medium transition-colors" style={{ color: 'var(--accent)' }}>
          Read full report →
        </Link>
      </div>
    </article>
...
function WireCard({ article, index }: { article: GeneratedArticle; index: number }) {
  return (
    <article className="group flex items-start gap-4 py-4" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
      <span className="font-serif text-2xl font-bold flex-shrink-0 w-8 text-right leading-none mt-1" style={{ color: 'var(--foreground-dim)', opacity: 0.4 }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <Link href={`/article/${article.slug}`} className="block">
          <h4 className="font-serif font-bold leading-snug mb-1 transition-colors group-hover:text-yellow-400" style={{ color: 'var(--foreground)' }}>
            {article.headline}
          </h4>
        </Link>
  ...
  return (
    <div className="glass-panel p-4 mb-6">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--foreground-muted)', fontFamily: 'monospace' }}>
          Editorial Status
        </span>
        <span className="ml-auto text-xs" style={{ color: 'var(--foreground-dim)', fontFamily: 'monospace' }}>
          {count} reports live
        ...
        {steps.map((step, i) => (
          <div key={step.label} className="flex items-center gap-1 flex-1">
            <div className="flex-1">
              <div className="h-1 rounded-full" style={{ background: i < steps.length ? 'var(--accent)' : 'var(--border)' }} />
              <div className="flex justify-between mt-1">
                <span className="text-xs" style={{ fontFamily: 'monospace', fontSize: '0.55rem', color: i < steps.length - 1 ? 'var(--accent)' : '#4ade80', letterSpacing: '0.05em' }}>
                  {step.label}
                </span>
              </div>
            </div>
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Poll every 4 seconds while loading
  ...
  // Also re-trigger fetch when pollCount changes (coverage redundancy)
  useEffect(() => {
    if (!loading) return;
    fetchArticles();
  }, [pollCount, fetchArticles, loading]);

  if (loading || articles.length === 0) {
    return <LoadingState count={articles.length} />;
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {hasBreaking && (
        <div className="flex items-center gap-3 px-4 py-2.5 rounded-lg mb-6" style={{ background: 'rgba(224, 82, 82, 0.1)', border: '1px solid rgba(224, 82, 82, 0.25)' }}>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-0.5 h-6 rounded-full" style={{ background: 'var(--accent)' }} />
            <h1 className="font-serif text-2xl font-bold" style={{ color: 'var(--foreground)' }}>Top Stories</h1>
          ...
          {secondary.length > 0 && (
            <div className="grid sm:grid-cols-2 gap-4">
              {secondary.map((a, i) => <SecondaryCard key={a.id} article={a} delay={(i + 1) * 100} />)}
            </div>
          )}

          {wireStories.length > 0 && (
            <div className="glass-panel p-5 animate-fade-in-up delay-300">
              ...
              {wireStories.map((a, i) => <WireCard key={a.id} article={a} index={i} />)}
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-5 rounded-xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            <h3 className="text-sm font-semibold mb-2" style={{ color: 'var(--foreground)' }}>About this publication</h3>
            <p className="text-xs leading-relaxed mb-3" style={{ color: 'var(--foreground-muted)' }}>
              AutoPress produces in-depth reporting through a fully automated editorial engine. No human editors. Every article is 700+ words with sources cited.
            </p>
      ...
      {moreStories.length > 0 && (
        <section className="mt-12">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {moreStories.map((a, i) => <SecondaryCard key={a.id} article={a} delay={i * 100} />)}
          </div>
        </section>
      )}
    </div>
